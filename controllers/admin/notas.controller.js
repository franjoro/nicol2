...
notas.getBoletaBimestral = async (req, res) => {
  try {
    ...
    const queryPromesas = [
      ...
      /* ESTUDIANTES */
      pool.query(
        `SELECT idAlumno , (SELECT CONCAT(Apellido,' ',Nombre) FROM alumnos WHERE Carnet = grado_alumno.idAlumno ) AS Nombre, ${columm} AS conducta FROM grado_alumno WHERE idGrado = ? AND idAlumno  = ?  ORDER BY Nombre`,
        [idGrado, idAlumno]
      ),
    ];
    const {
...
notas.getBoletaBimestralPreescolar = async (req, res) => {
  try {
    ...
    const queryPromesas = [
      ...
      /* ESTUDIANTES */
      pool.query(
        `SELECT idAlumno , (SELECT CONCAT(Apellido,' ',Nombre) FROM alumnos WHERE Carnet = grado_alumno.idAlumno ) AS Nombre, ${columm} AS conducta FROM grado_alumno WHERE idGrado = ?  ORDER BY Nombre`,
        [idGrado]
      ),
    ];
    const {
    ...
    await GenerarBoletaBimestralPreescolar(dataOrdenada, nombreGrado);
    res.json({ status: true });
    // const util = require('util');
    // console.log(util.inspect(dataOrdenada, false, null, true));
  } catch (error) {
...
notas.getBoletaBimestralPreescolarPorAlumno = async (req, res) => {
  try {
    ...
    const {
      ...
      "SELECT grados.id AS idGrado, grados.nombre AS nombreGrado FROM grados INNER JOIN year ON year.year = grados.idYear INNER JOIN grado_alumno ON grado_alumno.idGrado = grados.id WHERE year.estado = 1 AND idAlumno = ? GROUP BY idAlumno",
      [idAlumno]
    );
...
notas.getConsolidadoBimestralExcel = async (req, res) => {
  try {
    ...
    const queryPromesas = [
      ...
      /* ESTUDIANTES */
      pool.query(
        `SELECT idAlumno , (SELECT CONCAT(Apellido,' ',Nombre) FROM alumnos WHERE Carnet = grado_alumno.idAlumno ) AS Nombre, ${columna} AS conducta FROM grado_alumno WHERE idGrado = ? GROUP BY idAlumno  ORDER BY Nombre `,
        [idGrado]
      ),
    ];
    const {
...
notas.getNombreGradoPorIdAlumno = async (req, res) => {
  try {
    ...
    const {
      ...
      "SELECT grados.nombre AS nombreGrado FROM grado_alumno INNER JOIN grados ON grado_alumno.idGrado = grados.id WHERE grado_alumno.idAlumno = ? AND grados.idYear = (SELECT year FROM year WHERE year.estado = 1); ",
      [idAlumno]
    );
